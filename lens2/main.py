...
def create_board_yaml(lens_meta):
    ...
    for table_info in lens_meta['tables']:
        ...
        if table_info['type'] == 'view' and table_info['public']:
            print(f"✅ Processing {table_info['type']}:{name}")

            if table_info['viewType'] == 'metric':
                ...
                if len(table_info.get('dimensions', [])) != 0:
                    for dimension in table_info['dimensions']:
                        ...
                        time_dimension = dimension_name if dimension['type'] == 'time' and time_dimension == "" \
                            else time_dimension
                        dimensions.append({
                    ...
                    for measure in table_info.get('measures', []):
                        ...
                        for measure_dim_name in measure_dims_columns:
                            ...
                            if m_d_name_with_table not in existing_dimensions:
                                ...
                                print(f"⚠️ Skipped measure - `{measure_name}`: `{table_info['name']}`")
                                break
                        if is_skip is False:
                            measures.append({
                    ...
                    additional_kv = {}
                    excludes_dimension = []
                    if 'metric' in meta_info.keys():
                        metric_meta = meta_info.get('metric', {})
                        if "excludes" in metric_meta.keys():
                            if is_prefix:
                                excludes_dimension = [d.replace(".", "_") for d in metric_meta['excludes']]
                            else:
                                excludes_dimension = [d.split(".")[1] for d in metric_meta['excludes']]
                            del metric_meta['excludes']
                        additional_kv['available_time_zones'] = lens_meta.get("timeZones", "")
                        additional_kv['timeseries'] = time_dimension
                        additional_kv['refresh'] = {'cron': metric_meta['expression']}
            ...
            elif (table_info['viewType'] == 'entity' and 'export_to_board' in meta_info.keys() and meta_info[
                'export_to_board'] is True) \
                    or (table_info['viewType'] == 'entity' and 'export_to_iris' in meta_info.keys()
                        and meta_info['export_to_iris'] is True):
